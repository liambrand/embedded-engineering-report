...
\end{lstlisting} 
	...
	\chapter{Reliability}
		...
		\section{Section 2}
			...
			\subsection{(c)}
				\subsubsection{Voter System}
				A voter implementation was attempted but ultimately failed, due to the occassional occurrence of an out of bounds error. It can be viewed in Appendix \ref{voter}. The logic behind it involves the use of a checker function, rather than acquiring an average of readings, each reading in the sensor array would be collected and sent to the checker. The checker would iterate through the list, taking each reading and removing it. It would average all other readings that were taken, and see if the difference between the reading in question and the average of the other readings exceeded twice the sensor noise value. If it did, its location within the array would be flagged. It would then be purged from the array, and the readings minus the value identified as erroneous would be returned where the program would continue as normal.
			
				\subsubsection{Evaluation}
				As can be seen from the results of the solo sensor and the sensor array tests, the standard deviation with a majority of faulty sensors manages to remain much lower than the standard deviation of a single faulty sensor. The average value approach's general lower deviation also shows that the impact of noise on the sensor readings has been reduced somewhat as well.
	...
	\chapter{Voter Function Attempt}
	\label{voter}
	\lstinputlisting[caption=voter.java, language=java, showstringspaces=false]{voter.java}
	\end{appendices}
